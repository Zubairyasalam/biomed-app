...
@section('header_title', 'About Organizer Settings')

@section('content')
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h2 style="color: var(--admin-sidebar); font-size: 1.5rem;">About Organizer</h2>
        <p style="color: #64748b; font-size: 0.9rem;">Manage the content for the About Organizer page.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.about_organizer.update') }}" enctype="multipart/form-data">
        @csrf
        <div style="display: grid; grid-template-columns: 1fr; gap: 30px;">
            
            @foreach(['about_organizer' => 'About Organizer Content'] as $groupKey => $groupName)
                        ...
                        @foreach($settings[$groupKey] as $setting)
                            <div style="margin-bottom: 20px;">
                                ...
                                @if($setting->type == 'textarea')
                                    <textarea name="{{ $setting->key }}" style="width: 100%; padding: 12px 15px; border: 1px solid var(--admin-border); border-radius: 8px; font-family: 'Poppins', sans-serif; font-size: 0.95rem;" rows="15">{{ old($setting->key, $setting->value) }}</textarea>
